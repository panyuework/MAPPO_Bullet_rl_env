...
import numpy as np
from envs.env_core_discrete import EnvCore
...
class DiscreteActionEnv(object):
    ...
    def __init__(self):
        ...
        for agent in range(self.num_agent):
            ...
            # observation space
            share_obs_dim = [self.num_agent * 100, 100, 5]

            self.observation_space.append(
                spaces.Box(
                    low=-np.inf,
                    high=+np.inf,
                    shape=tuple(self.signal_obs_dim,),
                    dtype=np.float32,
                )
            )  # [-inf,inf]
        ...
        self.share_observation_space = [
            spaces.Box(
                low=-np.inf, high=+np.inf, shape=tuple(share_obs_dim,), dtype=np.float32
            )
